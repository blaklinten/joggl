...
/**
 * This class is used to import already existing entries into the database. The existing entries
 * must be saved according to the schema used in the trackMe-project found at
...
/**
 * This class is used to list all files in the file system. By listing all files (and folders) the
 * program can gather the necessary information to create EntryDTO entries to save to the database.
 */
class ListFiles extends SimpleFileVisitor<Path> {
  ...
  /**
   * An enum that is used to keep track on what EntryDTO-property the current directory represent.
   */
  ...
  /** Public constructor that returns a ListFiles-instance with indentionLevel set to 0. */
  public ListFiles() {
    indentionLevel = 0;
  }
  ...
  /**
   * This method is run whenever a directory is fully explored and it is time to take a step up in
   * the directory tree.
   *
  ...
  public FileVisitResult preVisitDirectory(Path directory, BasicFileAttributes attributes)
    ...
    } else if (directory.getNameCount() < oldDirDepth
        && currentDirectoryLevel == DirectoryLevel.Project) {
      client = directory.getFileName().toString();
      System.out.println("Setting Client to " + client);
    }
